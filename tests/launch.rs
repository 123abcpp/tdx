...
fn launch() {
    ...
    const CODE_MEM_ADDRESS: usize = 0xFFFFF000;

    
    // create vm
    let mut kvm_fd = Kvm::new().unwrap();
    let (mut tdx_vm, vm_fd) = TdxVm::new(&kvm_fd, 100).unwrap();
    let caps = tdx_vm.get_capabilities(&vm_fd).unwrap();
    println!("Caps:{:?}", caps);
    let mut cpuid = tdx_vm.init_vm(&kvm_fd, &caps, &vm_fd).unwrap();
    ...
    let code_gmem = linux_ioctls::create_guest_memfd(&vm_fd, &code_gmem);
    if code_gmem < 0 {
    ...
    linux_ioctls::set_user_memory_region2(&vm_fd, &code_mem_region);
    ...
    */
    ...
    loop {
        match tdx_vcpu.fd.run().expect("run failed") {
            ...
            e => panic!("Unexpected exit reason: {:?}, ret-reson:{:?}", errno::Error::last(), e),
        }
    }
}
